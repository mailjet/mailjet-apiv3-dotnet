...
namespace Mailjet.Repositories.Repositories.MailJet
{
    public class JobMailJetRepository : MailJetRepositoryBase, IJobRepository
    {
        ...
        public JobDataContract CreateMergeContactsIntoListJob<T>(Int64 contactslistId, IList<ContactBasicDataContract<T>> contacts)
            where T : class, new()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                JobDataContract job = this.ReadContactListJobStatus(jobId.JobID, contactslistId);
